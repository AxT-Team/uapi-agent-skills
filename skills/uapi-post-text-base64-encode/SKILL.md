---
name: uapi-post-text-base64-encode
description: "使用 UAPI 的“Base64 编码”单接口 skill，处理 Base64 编码、Base64编码 等请求。 Use when the user wants post text base64 encode, text base64 encode, encode, base64 encode, base64 decode, text analysis, or when you need to call POST /text/base64/encode directly."
---
...
- 方法：`POST`
- 路径：`/text/base64/encode`
- 分类：`Text`
- Operation ID：`post-text-base64-encode`

## 什么时候直接选这个 skill

- 当用户的目标和“Base64 编码”完全对应时，优先直接选它。
...
2. 再读 `references/operations/post-text-base64-encode.md`，确认参数、请求体、默认值、生效条件和响应码。
...
- `references/operations/post-text-base64-encode.md`：这里是调用前必须看的核心接口页。
