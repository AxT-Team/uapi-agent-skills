---
name: uapi-get-webparse-extractimages
description: "使用 UAPI 的“提取网页图片”单接口 skill，处理 提取网页图片、网页转Markdown、网页元数据、网页解析 等请求。 Use when the user wants get webparse extractimages, webparse extractimages, extractimages, or when you need to call GET /webparse/extractimages directly."
---
...
- 方法：`GET`
- 路径：`/webparse/extractimages`
- 分类：`WebParse`
- Operation ID：`get-webparse-extractimages`

## 什么时候直接选这个 skill

- 当用户的目标和“提取网页图片”完全对应时，优先直接选它。
...
2. 再读 `references/operations/get-webparse-extractimages.md`，确认参数、请求体、默认值、生效条件和响应码。
...
- `references/operations/get-webparse-extractimages.md`：这里是调用前必须看的核心接口页。
